/*!
 * nuxt-edge v2.0.0-25629050.054ea79 (c) 2016-2018
...
const r = function r() {
  ...
  const lastArg = ___default.last(args);
...
const createRoutes = function createRoutes(files, srcDir, pagesDir) {
  ...
  files.forEach((file) => {
    ...
    keys.forEach((key, i) => {
      ...
      const child = ___default.find(parent, { name: route.name });
      if (child) {
...
Options.from = function (_options) {
  ...
  options.modulesDir = []
    ...
    .concat(path__default.join(options.nuxtDir, 'node_modules'))
    .filter(hasValue)
    ...
    for (const dir of [this.srcDir, this.rootDir]) {
      for (const file of [
        ...
        if (fs.existsSync(path__default.resolve(dir, file))) {
          const postcssConfigPath = path__default.resolve(dir, file);
          return {
            sourceMap: this.cssSourceMap,
            config: {
